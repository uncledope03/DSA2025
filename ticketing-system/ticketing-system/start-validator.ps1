...
Write-Host "Starting Validator Service..." -ForegroundColor Green
Write-Host "Changing to validator-service directory..." -ForegroundColor Yellow

Set-Location "C:\ticketing-system\validator-service"

Write-Host "Running Ballerina service..." -ForegroundColor Yellow
try {
    bal run main.bal
} catch {
    Write-Host "Error starting validator service: $_" -ForegroundColor Red
...
Write-Host "Validator Service stopped." -ForegroundColor Red
