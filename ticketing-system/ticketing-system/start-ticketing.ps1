...
Write-Host "Starting Ticketing Service..." -ForegroundColor Green
Write-Host "Changing to ticketing-service directory..." -ForegroundColor Yellow

Set-Location "C:\ticketing-system\ticketing-service"

Write-Host "Running Ballerina service..." -ForegroundColor Yellow
try {
    bal run main.bal
} catch {
    Write-Host "Error starting ticketing service: $_" -ForegroundColor Red
...
Write-Host "Ticketing Service stopped." -ForegroundColor Red
