...
Write-Host "Changing to payment-service directory..." -ForegroundColor Yellow

Set-Location "C:\ticketing-system\payment-service"

Write-Host "Running Ballerina service..." -ForegroundColor Yellow
try {
    bal run main.bal
} catch {
    Write-Host "Error starting payment service: $_" -ForegroundColor Red
